...
def get_scouting_report(player_name, position, age, team, url):
...
def radar_chart_from_url(url, name):
    ...
    plt.title(f"{name} Traits Radar Chart", fontsize=14, fontweight='bold', color='#02FF9D', pad=20)
...
def main():
    ...
    if st.button("Generate Scouting Report"):        
        ...
        scouting_report = get_scouting_report(player_info.get('Player_Name', 'N/A'), position, age, team, githubLink)
